...
class class_database(object):
        ...
        def get_instructor_info(self, class_name):
                ...
                for row in c.execute("SELECT {first}, {last}, {email} FROM {ctn} WHERE {cn} = \'{class_name}\';".\
                        format(first='first', last='last', email='email', ctn=self.ctn, cn= self.class_name, class_name=class_name)):
                        info = {
