...
def parse_args():
        ...
        parser.add_argument('-c', '--create_class', dest='class_info', nargs='+',
                help = "Create new Jupyterhub docker. Memory limit [number][unit], units = b, k, m or g, e.g. 20G. Number of weighted CPU shares, set to 1024 if unsure. version is either latest or v*.* \n Format: python manage_class.py --create_class <class_name> <first> <last> <email> <username> <host> <version> <memory_limit> <cpu_shares>")
...
def create_class(args, db, cli):
        ...
        info = {'class_name':args.class_info[0],
                ...
                'jupyter_port':db.get_first_available_port(args.class_info[5]),
                'r_port':db.get_first_available_port(args.class_info[5]),
                'version':args.class_info[6],
                'mem_limit':args.class_info[7],
                'cpu_shares':args.class_info[8]
                }
        create.create_class(info, cli)
        #Update the class database with the new class
        db.insert_class(info['host'], info['jupyter_port'], info['class_name'], info['user'], info['first'], info['last'], info['email'])
